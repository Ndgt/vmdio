...
void printSummaryData(const vmd::VMDData &pVmdData)
{
    ...
    std::cout << "Light Frames Count: " << pVmdData.lightFrames.size() << "\n";
...
void printCameraFrames(const vmd::VMDData &pVmdData)
{
    ...
    {
        vmd::Position lPos = lCameraFrame.position;
        vmd::CameraRotation lRot = lCameraFrame.rotation;
        vmd::CameraInterpolation lInterp = lCameraFrame.interpolation;
...
void printLightFrames(const vmd::VMDData &pVmdData)
{
    ...
    {
        vmd::Color lColor = lLightFrame.color;
        vmd::Position lPos = lLightFrame.position;
...
void printSelfShadowFrames(const vmd::VMDData &pVmdData)
{
    ...
    for (const auto &lSelfShadowFrame : pVmdData.selfShadowFrames)
    {
        ...
        std::cout << "  SelfShadow Mode: " << lSelfShadowModeStr << "\n";
        std::cout << "\n";
    }
}
...
int main(int argc, char *argv[])
{
    try
    {
        ...
        // Read VMD file and populate the data structure
        vmd::VMDData lVmdDataForCameraEdit = vmd::readVMD(lVMDFilePath);
